...
const Brands = () => {
  ...
  return (
    <section className="section-shell py-12 md:py-14">
      <div className="glass-panel rounded-3xl px-6 py-8 md:px-12 md:py-12 border border-[var(--panel-border)]">
        ...
        <div className="hidden md:grid md:grid-cols-2 gap-6 lg:gap-8 items-stretch">
          {brands.map((brand) => (
            <div
              key={brand.id}
              className="theme-card h-[170px] rounded-2xl flex items-center justify-center px-6"
            >
              <img
                src={brand.logo}
                alt={brand.name}
          ...
          <Slider {...mobileSettings}>
            {brands.map((brand) => (
              <div key={brand.id} className="px-2">
                <div className="theme-card h-[130px] rounded-2xl flex items-center justify-center px-6">
                  <img
                    src={brand.logo}
                    alt={brand.name}
                    className={`w-full ${brand.widthClass} object-contain`}
